...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const title = isEn 
        ? 'Private Driver Agadir – Professional Chauffeur Service | Mdina Tours' 
        : 'Chauffeur Privé Agadir – Voiture avec Chauffeur au Maroc | Mdina Tours';
    const description = isEn
        ? 'Book a professional private driver in Agadir. Premium sedans, minivans, and SUVs with English-speaking local chauffeurs (Dispo Chauffeur) for resort transfers and coastal day trips.'
        : 'Louez un véhicule avec chauffeur privé à Agadir. Chauffeurs professionnels bilingues (Disposition Chauffeur) pour vos excursions côtières et transferts.';
    const url = `https://mdinatours.com/${lang}/private-driver-agadir`;
    ...
    return {
        ...
        alternates: {
            canonical: url,
            languages: {
                'en': 'https://mdinatours.com/en/private-driver-agadir',
                'fr': 'https://mdinatours.com/fr/private-driver-agadir',
            },
        },
        openGraph: {
            title,
...
export default async function PrivateDriverAgadirPage({ params }: { params: Promise<{ lang: string }> }) {
    const { lang } = await params;
    ...
    const textAgadir = {
        h1: isEn ? "Private Driver in Agadir – Professional Chauffeur Service" : "Chauffeur Privé à Agadir – Service de Chauffeur de Disposition",
        subtitle: isEn 
            ? "Explore Morocco's premier beach destination, Taghazout, and Paradise Valley at your own pace with a dedicated, English-speaking driver. Fixed daily rates."
            : "Explorez la première station balnéaire du Maroc, Taghazout et Paradise Valley à votre rythme avec un chauffeur privé bilingue à votre entière disposition. Tarifs fixes.",
        bannerLabel: isEn ? "Agadir Driver" : "Chauffeur Agadir",
        whatsappHeroMsg: isEn 
            ? "Hello Mdina Tours, I would like to book a private driver service in Agadir."
            : "Bonjour Mdina Tours, je souhaite réserver un chauffeur privé à Agadir.",
        introTitle: isEn ? "Premium Agadir Chauffeur & Dispo Service" : "Service de Chauffeur Privé & Disposition Premium à Agadir",
        introP1: isEn
            ? "For travelers visiting Agadir, navigating Morocco's leading seaside resort and the surrounding Souss region can be a highlight of your holiday. Our professional private driver dispo service (locally called \"Dispo Chauffeur\") offers you complete independence. Avoid negotiating with local small taxis or tour buses. Enjoy a premium vehicle and a dedicated chauffeur on standby to take you to the beach resorts of Taghazout, the breathtaking Paradise Valley, or on transfers to Marrakech."
            : "Pour les voyageurs de passage à Agadir, se déplacer dans la première station balnéaire du Maroc et la région du Souss peut s'avérer exceptionnel. Notre service de chauffeur privé à disposition (« Dispo Chauffeur ») vous offre une indépendance totale. Oubliez le stress des négociations de taxis locaux. Disposez d'un véhicule récent et d'un chauffeur en attente pour vous rendre à Taghazout, Paradise Valley ou pour vos transferts vers Marrakech.",
        faqTitle: isEn ? "Agadir Chauffeur Dispo FAQs" : "Questions Fréquentes - Chauffeur Dispo à Agadir",
        useCasesTitle: isEn ? "Popular Agadir Chauffeur Use Cases" : "Exemples d'excursions avec chauffeur à Agadir"
    };
    ...
    const itineraries = [
        {
            title: isEn ? "Agadir Al-Massira Airport & Hotel Transfers" : "Transferts Aéroport Agadir Al-Massira & Hôtels",
            desc: isEn 
                ? "Stress-free transfers between Agadir Airport (AGA) and Taghazout, Essaouira, or local beach hotels."
                : "Transferts sans stress entre l'Aéroport d'Agadir (AGA) et Taghazout, Essaouira ou les hôtels balnéaires.",
        ...
        {
            title: isEn ? "Taghazout & Paradise Valley Tour" : "Excursion Taghazout & Paradise Valley",
            desc: isEn 
                ? "Ride into the Atlas foothills to see Paradise Valley palm groves, then visit the surf haven of Taghazout."
        ...
        {
            title: isEn ? "Agadir to Marrakech Transfer / Day Trip" : "Transfert / Excursion d'Agadir à Marrakech",
            desc: isEn 
                ? "Travel comfortably on the modern expressway from the coast to the Red City of Marrakech."
    ...
    const reviews = [
        {
            quote: isEn ? (
                <>Our driver in Agadir was excellent. He drove us to Paradise Valley and waited for hours while we hiked. The car was very clean and had cold water. <strong style={{ fontWeight: 800 }}>Highly recommend!</strong></>
            ) : (
                <>Notre chauffeur à Agadir était excellent. Il nous a conduits à Paradise Valley et a attendu pendant notre randonnée. Voiture propre avec eau fraîche. <strong style={{ fontWeight: 800 }}>Hautement recommandé !</strong></>
            ),
            author: "Emma S.",
            flag: "🇺🇸"
        },
    ...
    const faqs = [
        {
            q: isEn ? "What is a private driver (\"dispo chauffeur\") in Agadir?" : "Qu'est-ce qu'un service de chauffeur privé (« dispo chauffeur ») à Agadir ?",
            a: isEn 
                ? "A dispo chauffeur is a private vehicle hired with a professional driver at your full disposal. Perfect for exploring coastal locations, surf towns like Taghazout, or doing transfers without fixed schedules."
                : "La formule « dispo chauffeur » désigne la location d'un véhicule avec chauffeur à votre entière disposition. Idéal pour explorer la côte, Taghazout, ou faire vos trajets sans horaire rigide."
        },
        {
            q: isEn ? "Can I book a driver from Agadir-Al Massira Airport (AGA)?" : "Puis-je réserver un chauffeur depuis l'Aéroport d'Agadir-Al Massira (AGA) ?",
            a: isEn
                ? "Yes, you can schedule a pickup directly. Your driver will track flight landings, wait in the arrivals hall with a name sign, and take you to your hotel or start your dispo schedule."
                : "Oui, vous pouvez planifier une prise en charge à l'aéroport AGA. Le chauffeur suivra votre vol et vous accueillera dans le hall avec une pancarte."
        },
        {
            q: isEn ? "Can we travel from Agadir to Marrakech or Essaouira?" : "Peut-on voyager d'Agadir vers Marrakech ou Essaouira ?",
            a: isEn
                ? "Yes, Marrakech and Essaouira are highly popular destinations easily accessible from Agadir. Marrakech is roughly 3 hours via the expressway, while Essaouira is 2.5 hours via the scenic coastal road."
                : "Oui, Marrakech et Essaouira sont des destinations très demandées. Marrakech est à environ 3h via l'autoroute, et Essaouira à 2h30 par la route côtière."
        },
        {
            q: isEn ? "Are fuel and highway tolls included in the price?" : "Le carburant et les péages sont-ils inclus ?",
            a: isEn
                ? "Yes. All daily rates include fuel, highway tolls, parking costs, and the driver's fees. No hidden or surprise surcharges."
                : "Oui. Tous nos tarifs journaliers incluent le carburant, les péages d'autoroute, les frais de stationnement et la rémunération du chauffeur. Aucun frais caché."
        },
        {
            q: isEn ? "Do the drivers speak English?" : "Les chauffeurs parlent-ils anglais ?",
            a: isEn
                ? "Yes, our drivers speak fluent English and French. They are experienced and professional chauffeurs focused on providing high-quality service."
                : "Oui, nos chauffeurs parlent couramment anglais et français. Ils sont professionnels, expérimentés et soucieux de votre confort."
        }
    ];
    ...
    const breadcrumbJsonLd = {
        ...
        "itemListElement": [
            ...
            },
            {
                ...
                "position": 3,
                "name": "Agadir",
                "item": `https://mdinatours.com/${language}/private-driver-agadir`
            }
        ]
    };
            ...
            <main style={{ backgroundColor: 'var(--bg-color)', minHeight: '100vh', paddingTop: '100px' }}>
                <div className="breadcrumbs-title-container" style={{ maxWidth: '1150px', margin: '0 auto', padding: '0 20px 20px 20px' }}>
                    <nav className="breadcrumb-nav" style={{ display: 'flex', gap: '8px', fontSize: '0.9rem', color: '#666', marginBottom: '15px' }}>
                        ...
                        <span style={{ color: 'var(--accent)', fontWeight: 500 }}>{textAgadir.bannerLabel}</span>
                    </nav>
                    ...
                    <div className="ratings-badges-row" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', fontSize: '0.85rem', marginBottom: '20px' }}>
                        <div className="rating-pill" style={{ display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: '#f1f5f9', padding: '6px 12px', borderRadius: '20px' }}>
                            ...
                            <span style={{ color: '#717171', fontWeight: 500 }}>(31 {isEn ? "reviews" : "avis"})</span>
                        </div>
                ...
                <section id="booking" className="main-booking-section" style={{ maxWidth: '1150px', margin: '0 auto', padding: '0 20px 40px 20px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '30px' }} className="grid-responsive-layout">
                        <div className="transfers-content-col" style={{ display: 'flex', flexDirection: 'column' }}>
                            <PrivateDriverHeroGallery language={language} city="Agadir" title={textAgadir.h1} />
                            <div className="mobile-booking-widget" style={{ marginTop: '20px' }}>
                                <PrivateDriverBookingWidget language={language} defaultCity="Agadir" defaultDays={1} />
                            </div>
                ...
                <ChauffeurDestinations lang={language} pageType="agadir" />
